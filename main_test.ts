...
Deno.test(function round3Test2() {
	// this test was made to address:
	// when a node has 2 parents, it stores the teams from those parents
	// in fromUpperParent or fromLowerParent
	// the problem is that lets say round 1, 2, 3, and 4 are filled up
	// round 5 is calculated. round 5 nodes have fromXXXparent filled
	// next, let's say that round 2 is changed so that there is a draw
	// the data of future rounds is wiped
	// except the data in fromXXXparent is not wiped
	// this means that upon filling out of the parents of a 2 parent node
	// that round is already calculated using old data
	// so if you filled out round 2, 3.
	// and then as soon as you filled out round4 upper,
	// round 5 is calculated because it has the previous results
	// simply adding on code to remove fromXXXparent in clearDependents
	// will not work because it will inadvertantly remove the opposite 
	// fromXXXparent data
	// a better solution would be to have nodes keep track of references to
	// its parents. that way, the fromXXXparent variables are no longer needed
	// it is better to derive data, then to store a copy that can get out of sync
	const swissBracket = new SwissBracket(16, 3);
	swissBracket.setMatchRecordWithValue("0-0", 0, 3, 0);
	swissBracket.setMatchRecordWithValue("0-0", 1, 0, 2);
	swissBracket.setMatchRecordWithValue("0-0", 2, 4, 3);
